...
#SBATCH --output=slurm-%j_3b_instruct_critic_train.out
#SBATCH --error=slurm-%j_3b_instruct_critic_train.err
...
CRITIC_INIT_CKPT="/work/09576/shuozhe/saved_model/Qwen2.5-3B"
...
sync_to_work() {
  echo "Syncing PPO logs to WORK without training checkpoints..."
  mkdir -p "$ARCHIVE_DIR"
  if [[ ! -d "$RUN_DIR" ]]; then
  ...
  rsync -a \
    --exclude='train_log/global_step_*/' \
    --exclude='train_log/latest_checkpointed_iteration.txt' \
    "$RUN_DIR"/ "$ARCHIVE_DIR"/ || true
  echo "Archived PPO logs and non-checkpoint outputs to: $ARCHIVE_DIR"
  echo "Training checkpoints remain only on SCRATCH at: $TRAIN_LOG_DIR"
}
