#!/usr/bin/env python3
"""
Submit 18 SLURM jobs for PPO checkpoint sweep:
  6 checkpoint steps × 3 seeds = 18 jobs.

Actor and critic are co-located at the same checkpoint directory
(qwen2.5_7B_ppo_664998/global_step_NNN/{actor,critic}).

Usage:
    python submit_ppo_ckpt_sweep.py            # dry run: write scripts, print paths
    python submit_ppo_ckpt_sweep.py --submit   # actually submit to SLURM
"""
...
PPO_RUN_BASE = "/scratch/10587/npg493/verl_runs/qwen2.5_7B_ppo_664998"

CKPT_STEPS = [150, 300, 450, 600, 750, 900]
...
ARCHIVE_ROOT    = "${SCRATCH}/value_decoding_output_archive"
...
def ckpt_dir(step: int) -> str:
    return f"{PPO_RUN_BASE}/global_step_{step}"


def make_run_name(step: int, seed: int) -> str:
    return f"ppo_ckpt_step{step}_seed{seed}"
...
def render_script(
    ...
    script = textwrap.dedent(f"""\
        ...
        # Paths
        # -----------------------------
        # Actor and critic both come from the same PPO checkpoint step.
...
def build_jobs():
    """Yield (run_name, actor_dir, critic_dir, seed) for all 18 jobs."""
    for step in CKPT_STEPS:
        base = ckpt_dir(step)
        actor_dir  = base   # points to global_step_NNN; actor/ subdir resolved by the eval script
        critic_dir = base   # same step, critic/ subdir
        for seed in SEEDS:
            run_name = make_run_name(step, seed)
            yield run_name, actor_dir, critic_dir, seed, step


def main():
    parser = argparse.ArgumentParser(
        description="Submit PPO checkpoint sweep jobs to SLURM (6 steps × 3 seeds = 18 jobs)."
    )
    parser.add_argument("--submit", action="store_true",
                        help="Actually submit via sbatch (default: dry run).")
    parser.add_argument("--script-dir", default="slurm_scripts_ppo",
    ...
    print(f"{'=' * 60}")
    print(f"Total jobs: {len(jobs)}  (6 ckpt steps × 3 seeds)")
    print(f"PPO run base: {PPO_RUN_BASE}")
    print(f"Steps: {CKPT_STEPS}")
    print(f"Seeds: {SEEDS}")
    print(f"{'=' * 60}\n")

    submitted = []
    for run_name, actor_dir, critic_dir, seed, step in jobs:
        output_subdir = f"ppo_step{step}_seed{seed}"
