...
class CandidateExample:
    # State tokens: prompt_text + prefix_text_before_chunk (shared by all
    # candidates in the group — the critic scores this prefix, not the chunk).
    state_input_ids: tuple[int, ...]
    # Per-candidate mc_reward, kept for eval metrics / ranking diagnostics.
    target_reward: float
    # Mean mc_reward across all candidates in the group — the regression target.
...
class SearchInducedCriticDataset(Dataset):
    def __init__(
        ...
        *,
        ...
        self._compute_group_reward_means()
    ...
    def _load(self) -> None:
        ...
        with self.data_path.open("r", encoding="utf-8") as handle:
            for line_number, line in enumerate(handle, start=1):
                ...
                state_ids = self._row_state_input_ids(row, line_number=line_number)
                if not state_ids:
                    raise ValueError(f"Row {line_number} in {self.data_path} produced an empty state input sequence")
                if len(state_ids) > self.max_seq_length:
                    state_ids = state_ids[-self.max_seq_length:]
                ...
                # group_reward_mean is a placeholder (0.0) until _compute_group_reward_means runs.
                example = CandidateExample(
                    state_input_ids=tuple(int(t) for t in state_ids),
    ...
    def _row_state_input_ids(self, row: dict[str, Any], *, line_number: int) -> list[int]:
        """Tokenize prompt_text + prefix_text_before_chunk — the state the critic scores."""
        prompt_text = row.get("prompt_text")
        prefix_before_chunk = row.get("prefix_text_before_chunk")
        if not isinstance(prompt_text, str) or not isinstance(prefix_before_chunk, str):
            raise ValueError(
                f"Row {line_number} in {self.data_path} is missing prompt_text or "
                "prefix_text_before_chunk, which are required for state tokenization."
            )
        return [
            int(t)
            for t in self.tokenizer(
                prompt_text + prefix_before_chunk,
                return_attention_mask=False,
            )["input_ids"]
        ]

    def _compute_group_reward_means(self) -> None:
        """Replace the placeholder group_reward_mean=0.0 with the true group mean."""
        # CandidateExample is frozen, so we rebuild the list with correct means.
        updated: list[CandidateExample] = []
        for group_id, indices in self.group_to_indices.items():
            mean_reward = float(
                np.mean([self.examples[i].target_reward for i in indices])
            )
            for i in indices:
                ex = self.examples[i]
                updated.append((i, CandidateExample(
                    state_input_ids=ex.state_input_ids,
                    target_reward=ex.target_reward,
                    group_reward_mean=mean_reward,
                    prompt_id=ex.prompt_id,
                    candidate_group_id=ex.candidate_group_id,
                    search_step=ex.search_step,
                    selected_by_collector=ex.selected_by_collector,
                    collector_value=ex.collector_value,
                    group_has_mixed_rewards=ex.group_has_mixed_rewards,
                )))
        for index, example in updated:
            self.examples[index] = example

    def _build_sampling_indexes(self) -> None:
        rankable_set: set[int] = set()
        for group_id, indices in self.group_to_indices.items():
            rewards = [self.examples[i].target_reward for i in indices]
            has_mixed_rewards = len({float(r) for r in rewards}) > 1
            if has_mixed_rewards or any(self.examples[i].group_has_mixed_rewards for i in indices):
...
class SearchInducedBatchSampler(Sampler[list[int]]):
    ...
    def _mixed_batches(self, rng: random.Random) -> Iterator[list[int]]:
        ...
        for _ in range(self.num_batches):
            ...
            self._sample_from_pool(batch, self.dataset.positive_indices, rng, max(0, min(positive_count, self.batch_size - len(batch))))
            self._sample_from_pool(batch, self.dataset.negative_indices, rng, max(0, min(negative_count, self.batch_size - len(batch))))
            self._fill_random(batch, rng, self.batch_size)
            rng.shuffle(batch)
            if len(batch) == self.batch_size or (batch and not self.drop_last):
    ...
    def _sample_rankable_examples(self, rng: random.Random, count: int) -> list[int]:
        ...
        while len(batch) < count and group_ids:
            ...
            positives = [i for i in indices if self.dataset.examples[i].target_reward >= 0.5]
            negatives = [i for i in indices if self.dataset.examples[i].target_reward < 0.5]
        ...
        if len(batch) < count:
            rankable_indices = [i for gid in self.dataset.rankable_group_ids for i in self.dataset.group_to_indices[gid]]
...
def reduce_loss_dict_for_logging(loss_dict: dict[str, Any], distributed: DistributedContext) -> dict[str, Any]:
    ...
    tensor_keys = ("loss", "loss_mse", "loss_bce", "value_mean", "target_mean")
...
def summarize_batch_for_debug(batch: dict[str, Any]) -> dict[str, Any]:
    ...
    targets = batch["group_reward_mean"].detach().cpu().float().tolist()
    return {
        ...
        "candidate_group_id_sample": [str(gid) for gid in batch.get("candidate_group_id", [])[:8]],
    }
...
def collate_candidates(examples: Sequence[CandidateExample], *, pad_token_id: int) -> dict[str, Any]:
    """Collate a batch of CandidateExamples into model-ready tensors.

    Because all candidates sharing a candidate_group_id have identical state
    tokens (prompt + prefix_before_chunk), we deduplicate to one row per group
    for the forward pass.  The loss is therefore computed once per state, with
    group_reward_mean as the regression target.

    Per-candidate fields (target_reward, selected_by_collector, etc.) are kept
    for the first representative of each group so that eval metrics still work.
    """
    # Deduplicate: keep first occurrence of each candidate_group_id.
    seen_groups: set[str] = set()
    deduped: list[CandidateExample] = []
    ...
    max_length = max(len(ex.state_input_ids) for ex in deduped)
    input_ids = torch.full((len(deduped), max_length), int(pad_token_id), dtype=torch.long)
    attention_mask = torch.zeros((len(deduped), max_length), dtype=torch.long)

    for row_index, ex in enumerate(deduped):
        seq = torch.tensor(ex.state_input_ids, dtype=torch.long)
        input_ids[row_index, : seq.numel()] = seq
        attention_mask[row_index, : seq.numel()] = 1

    return {
        "input_ids": input_ids,
        "attention_mask": attention_mask,
        # group_reward_mean: regression target — mean mc_reward of all 8 candidates.
        "group_reward_mean": torch.tensor([ex.group_reward_mean for ex in deduped], dtype=torch.float32),
        # target_reward: per-candidate mc_reward of the representative row,
        # used only for eval metric logging.
        "target_reward": torch.tensor([ex.target_reward for ex in deduped], dtype=torch.float32),
        "prompt_id": [ex.prompt_id for ex in deduped],
        "candidate_group_id": [ex.candidate_group_id for ex in deduped],
        "search_step": torch.tensor([ex.search_step for ex in deduped], dtype=torch.long),
        "selected_by_collector": torch.tensor([ex.selected_by_collector for ex in deduped], dtype=torch.bool),
        "collector_value": [ex.collector_value for ex in deduped],
    }
...
def batch_to_device(batch: dict[str, Any], device: torch.device) -> dict[str, Any]:
    moved = dict(batch)
    for key in ("input_ids", "attention_mask", "target_reward", "group_reward_mean", "search_step", "selected_by_collector"):
...
def compute_loss(
    ...
    *,
    ...
    """Compute loss between critic state values and group-mean reward targets.

    values       : (N,) critic scalar output, one per deduplicated state.
    batch targets: group_reward_mean — mean mc_reward across all candidates in
                   the group, same shape (N,).
    """
    values_f = values.float()
    targets_f = batch["group_reward_mean"].float()
...
def compute_value_metrics(values: Sequence[float], rewards: Sequence[float]) -> dict[str, Any]:
    """Compute eval metrics for deduplicated state-level predictions.

    values  : critic scalar output, one per unique state (candidate group).
    rewards : group_reward_mean for each state — the same target used in training.
    """
...
def evaluate_critic(critic, dataloader: DataLoader, *, device: torch.device, max_examples: int | None = None) -> dict[str, Any]:
    ...
    for batch in tqdm(dataloader, desc="eval", leave=False):
        ...
        # Use group_reward_mean — the same target used during training.
        batch_rewards = batch["group_reward_mean"].detach().cpu().float().tolist()
        for value, reward in zip(batch_values_list, batch_rewards):
    ...
    metrics = compute_value_metrics(values, rewards)
    critic.train()
    return metrics
...
def write_plots(output_dir: Path) -> None:
    ...
    plot_specs = [
        ...
        ("eval_brier", "eval_brier_over_steps.png"),
        ("pearson", "pearson_over_steps.png"),
        ("spearman", "spearman_over_steps.png"),
        ("value_gap_correct_minus_wrong", "value_gap_over_steps.png"),
    ]
    ...
    for metric_name, filename in plot_specs:
        ...
        if not points:
            continue
        steps, metric_values = zip(*points)
        plt.figure()
...
def maybe_run_chunk_eval(args: argparse.Namespace, checkpoint_dir: Path, step: int, output_dir: Path) -> None:
    ...
    for seed_offset in range(num_seeds):
        ...
        cmd = [
            ...
            "--actor_checkpoint_dir", str(args.chunk_eval_actor_checkpoint_dir),
            "--critic_checkpoint_dir", str(checkpoint_dir),
            "--dataset_path", str(args.chunk_eval_dataset_path),
            "--output_dir", str(eval_output_dir),
            "--chunk_sizes", "128", "256",
            "--num_chunk_candidates_values", "8",
            "--generation_backend", str(args.chunk_eval_generation_backend),
