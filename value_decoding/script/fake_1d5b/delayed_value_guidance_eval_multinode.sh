#!/bin/bash
#SBATCH --job-name=delayed_value_guidance_1d5b
...
# Run identity
# -----------------------------
RUN_NAME="delayed_value_guidance_eval_1d5b"
...
OUTPUT_DIR="${RUN_DIR}/delayed_value_guidance_eval_1d5b"
...
# -----------------------------
nodes_array=()
SCRIPT_PATH="${WORK_DIR}/value_decoding/script/1d5b/$(basename "${BASH_SOURCE[0]}")"
...
# -----------------------------
# Run delayed-value-guidance eval
# -----------------------------
cd "$WORK_DIR"
...
CMD=(
  python3 -m value_decoding.delayed_value_guidance_eval
...
printf ' %q' "${CMD[@]}"
printf '\n'
"${CMD[@]}" 2>&1 | tee "$LOG_DIR/delayed_value_guidance_eval.log"

echo "Delayed value-guidance eval finished successfully."
