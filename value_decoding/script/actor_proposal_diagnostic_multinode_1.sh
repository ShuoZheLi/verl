#!/bin/bash
#SBATCH --job-name=actor_proposal_diag
...
# This is the per-node local worker layout.
# In the current 1-GPU-per-node Slurm setup, keep this as `cuda:0`.
# If you later move to multi-GPU nodes, update both WORKER_DEVICES and
# RAY_GPUS_PER_NODE.
...
CMD=(
  python3 -m value_decoding.actor_proposal_diagnostic
...
[[ "$STORE_TOKEN_ENTROPIES" != "0" ]] && CMD+=(--store_token_entropies)
[[ "$DISABLE_ACTOR_CACHE" != "0" ]] && CMD+=(--disable_actor_cache)

printf 'Running command:\n'
printf ' %q' "${CMD[@]}"
printf '\n'
"${CMD[@]}" 2>&1 | tee "$LOG_DIR/actor_proposal_diagnostic.log"

echo "Actor proposal diagnostic finished successfully."
